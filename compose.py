...
def compose():
	...
	with open('data/notes') as notesfile:
	...
	pred_output = compose(model, rnn_in, pitchnames, n_vocab)
	
	# Save midi file with composition
	save_midi(pred_output)
	
def prepare_sequences(notes: list, pitchnames: list, n_vocab: int, sequence_length = 100: int):
	""" Prepare RNN input """
...
def compose(model, rnn_in, pitchnames, n_vocab, n_notes = 500):
